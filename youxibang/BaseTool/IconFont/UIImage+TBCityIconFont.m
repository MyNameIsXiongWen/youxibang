...
+ (UIImage *)iconWithInfo:(TBCityIconInfo *)info {
    ...
    if ([info.text respondsToSelector:@selector(drawAtPoint:withAttributes:)]) {
         ...
         */
        if ([info.text isEqualToString:@"\U0000e610"]){
            [info.text drawAtPoint:CGPointMake(-2.5, 0) withAttributes:@{NSFontAttributeName:font, NSForegroundColorAttributeName: info.color}];
        }else{
