...
//这个页面基本和订单消息页面一样
@interface WalletMessageViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation WalletMessageViewController
...
- (void)viewDidLoad {
    ...
    UILabel* lab = [EBUtility labfrome:CGRectMake(0, 0, 220, 20) andText:@"暂无钱包消息" andColor:[UIColor darkGrayColor] andView:self.placeHoldView];
...
- (void)downloadData{
    ...
    [[NetWorkEngine shareNetWorkEngine] postInfoFromServerWithUrlStr:[NSString stringWithFormat:@"%@Message/walletmsglist.html",HttpURLString] Paremeters:dict successOperation:^(id object) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UILabel* addtime = [cell viewWithTag:1];
    addtime.text = [NSString stringWithFormat:@"%@",self.dataAry[indexPath.row][@"addtime"]];
    UILabel* title = [cell viewWithTag:2];
    title.text = [NSString stringWithFormat:@"%@",self.dataAry[indexPath.row][@"title"]];
    UILabel* user_money = [cell viewWithTag:3];
    user_money.text = [NSString stringWithFormat:@"%@",self.dataAry[indexPath.row][@"user_money"]];
    UILabel* content = [cell viewWithTag:4];
    content.text = [NSString stringWithFormat:@"%@",self.dataAry[indexPath.row][@"content"]];
    return cell;
}
